...
export async function fetchOwnBusiness(userId: string): Promise<VendorBusinessDetail | null> {
  const { data, error } = await supabase
    .from("businesses")
    .select(
      "id, name, business_type, category, description, address, contact, opening_hours, business_story, unique_specialty, accessibility_info, social_media_links, language, verification_status, review_notes, featured_status, registered_or_informal, views_count, saves_count"
    )
