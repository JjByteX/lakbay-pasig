...
// Size of the bottom-left box the animated SVG itself renders into, as a
// percentage of the PANEL (not the viewport) -- this is what actually
// keeps the artwork out of the panel's vertical/horizontal center, where
// the headline and CTAs sit, regardless of preserveAspectRatio. Raised
// again from 68/52 (direct instruction: "the waves a little more
// bigger"), re-checked against the now-centered copy block (see
// landing.tsx) rather than the old left-aligned one, since centering
// changes how close the text sits to this bottom-left corner. Still
// well under half the panel on each axis, so the corner placement holds
// even on a short/wide panel (mobile's stacked layout).
...
export function HeroLinesBackground() {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden bg-white">
      ...
      {/* Bottom-left box, not the full panel (h-full w-full) -- see this
          file's top comment for why the box's own size/position, not
          preserveAspectRatio alone, is what keeps the artwork out of the
          panel's center where the headline and CTAs sit. */}
      <div
        ref={containerRef}
