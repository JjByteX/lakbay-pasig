...
// Staff creation and edit, as a modal instead of a separate page/route.
// Was admin-staff-detail.tsx (isNew-branch page at /admin/staff/new and
// /admin/staff/:id) before this change; per ux-ui-guidelines.md's Modal
// vs panel rule, a focused form that fits a single viewport belongs in a
// centered modal, not its own route. All form logic (create via the
// create-staff-account Edge Function, edit via a direct profiles update,
// the 3.5 self-lockout guard) is unchanged from that page, moved as-is.
//
...
// single form is the whole dialog, per ux-ui-guidelines.md's Card
...
interface StaffFormDialogProps {
  ...
  /** Staff row id being edited, or null for New Staff Account. */
  staffId: string | null;
...
export default function StaffFormDialog({
  ...
  const isSelf = !isNew && staffId === profile?.id;
  ...
  // change after the dialog opens.
  async function guardSelfLockout(): Promise<string | null> {
    if (!isSelf || !staffId) return null;
    ...
    const others = await countOtherActiveAdmins(staffId);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
        ...
        {!loading && notFound && (
          <p className="text-sm text-destructive">Could not load this staff account.</p>
        )}
