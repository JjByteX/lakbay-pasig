...
- location-picker.tsx depends on geocode.ts (searchPlaces, reverseGeocode, formatAddress) and map-style.ts, and reuses discover-query.ts's Coordinates type and business-fields.tsx's FieldLabel. It is imported by admin-place-detail.tsx (step 1), and by business-fields.tsx, which in turn is shared by admin-business-detail.tsx and vendor-dashboard.tsx -- all three forms pick up any change to the picker or the geocoder at once. See decision-log.md entry #21
